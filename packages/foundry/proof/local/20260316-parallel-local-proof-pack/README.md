...
- preserves the original 3-instance `parallel-local` proof plus a stronger 5-instance host-local saturation result
- proves the repo-native matrix runner can coordinate **at least five fresh harness + Anvil deployments in parallel on one host** instead of only sequentially or only three at a time
...
- it proves bounded host-local overlap through 5 isolated instances on one machine, but it does **not** prove heavier 6-10 deployment saturation
