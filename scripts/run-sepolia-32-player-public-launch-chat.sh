...
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-base-sepolia-32p-public-launch-chat}"
ART_DIR="$ROOT/packages/foundry/canary/base-sepolia/$RUN_ID"
...
ensure_player_funded() {
  local idx="$1" addr current delta tx_hash
  addr="$(player_addr "$idx")"
  ...
  tx_hash="$(cast send "$addr" --value "$delta" --nonce "$FUNDING_NONCE" --rpc-url "$RPC_URL" --account "$OWNER_KS" --password-file "$OWNER_PW" --json | jq -r '.transactionHash')"
...
parallel_join_players() {
  local gid="$1" start_idx="${2:-2}" end_idx="${3:-$PLAYER_COUNT}" pids=() rc=0 idx cause
...
parallel_post_cause_messages() {
  ...
  for idx in $(seq 1 "$PLAYER_COUNT"); do
    ...
    (
      cd "$FOUNDRY_DIR"
      node scripts-js/gameCli.js post-cause --rpc-url "$RPC_URL" --game-id "$gid" --cause-id "$cause" --text "player-${idx} checking in for cause-${cause}; ready to coordinate before commit" --wallet-keystore "$(player_ks "$idx")" --wallet-keystore-password-file "$(player_pw "$idx")" --json > "$ART_DIR/game/post-cause-player-${idx}.json"
    ) &
    pids+=("$!")
    if (( ${#pids[@]} >= PARALLELISM )); then
      wait_batch "${pids[@]}" || rc=1
...
parallel_post_global_messages() {
  local gid="$1" start_idx="$2" end_idx="$3" prefix="$4" pids=() rc=0 idx
  for idx in $(seq "$start_idx" "$end_idx"); do
    (
      cd "$FOUNDRY_DIR"
      node scripts-js/gameCli.js post-global --rpc-url "$RPC_URL" --game-id "$gid" --text "${prefix}: player-${idx} online on latest contract and public-launch path" --wallet-keystore "$(player_ks "$idx")" --wallet-keystore-password-file "$(player_pw "$idx")" --json > "$ART_DIR/game/post-global-${prefix}-player-${idx}.json"
    ) &
    pids+=("$!")
    if (( ${#pids[@]} >= PARALLELISM )); then
      wait_batch "${pids[@]}" || rc=1
...
main() {
  ...
  log "Deploy latest contract candidate"
...
PY
  ...
  for idx in $(seq 1 "$CAUSE_COUNT"); do
    retry_capture_json "$ART_DIR/causes/whitelist-cause-${idx}.json" \
      node scripts-js/gameCli.js whitelist-cause --rpc-url baseSepolia --cause-id "$idx" --recipient "$(player_addr "$idx")" --metadata-text "32p-public-launch-cause-$idx" --wallet-keystore "$OWNER_KS" --wallet-keystore-password-file "$OWNER_PW" --json
  done

  log "Ensure 32 players are funded"
  ...
  for idx in $(seq 1 "$PLAYER_COUNT"); do
    ...
    node scripts-js/authCli.js permit --rpc-url baseSepolia --registry "$REG" --wallet "$(player_addr "$idx")" --agent-key-text "32p-public-launch-player-$idx" --manifest-uri "manifest://32p-public-launch/player-$idx" --ttl-seconds 14400 --nonce-text "32p-public-launch-player-$idx-${RUN_ID}" --verifier-keystore "$OWNER_KS" --verifier-keystore-password-file "$OWNER_PW" --out "$ART_DIR/auth/player-$idx/auth-permit.json"
  ...
  log "Public launch by player 1, then join players 2-32"
  ...
  parallel_join_players "$GID" 2 "$PLAYER_COUNT"

  log "Generate convincing chat volume"
  parallel_post_cause_messages "$GID"
  parallel_post_global_messages "$GID" 1 16 "join-phase"
  ...
  node scripts-js/queryCli.js messages --rpc-url baseSepolia --game-id "$GID" --from-block "$FROM_BLOCK" --json > "$ART_DIR/query/messages-after-joins.json"

  JOIN_DEADLINE=$(jq -r '.joinDeadline' "$ART_DIR/game/launch-game.json")
  wait_until_deadline "$JOIN_DEADLINE" "32-player public-launch join deadline"
  ...
  node scripts-js/queryCli.js messages --rpc-url baseSepolia --game-id "$GID" --from-block "$FROM_BLOCK" --json > "$ART_DIR/query/messages-final.json"
  node scripts-js/queryCli.js export --rpc-url baseSepolia --game-id "$GID" --from-block "$FROM_BLOCK" --out "$ART_DIR/query/game-1-export-final" --json > "$ART_DIR/query/export.json"

  cat > "$ART_DIR/operator-notes.md" <<EOF
# 32-Player Public Launch Chat Run

- Run ID: \
	t$RUN_ID
- Purpose: convincing latest-contract Base Sepolia evidence with 32 players and materially more chat.
- Candidate commit: $(git -C "$ROOT" rev-parse --short HEAD)
- Chain: Base Sepolia
- Launch path: public launch by admitted player wallet using launchGameAndJoin(...)
- Profile:
  - join ${PRISONERS_JOIN_DURATION_SECONDS}
  - commit ${PRISONERS_COMMIT_DURATION_BLOCKS}
  - reveal ${PRISONERS_REVEAL_DURATION_BLOCKS}
  - min ${PRISONERS_MIN_PLAYERS}
  - max ${PRISONERS_MAX_PLAYERS}
  - max causes ${PRISONERS_MAX_CAUSES}
  - entry fee 0.001 ETH
- Chat plan:
  - all 32 players posted cause-scoped messages
  - players 1-16 also posted global messages
- Final summary file: query/game-summary-final.json
- Final messages file: query/messages-final.json
EOF

  log "32-player public-launch chat run complete: $ART_DIR"
}
