...
- A preserved raw host-local saturation proof bundle now exists at `packages/foundry/proof/local/20260316-host-local-saturation-c7-proof/`, rooted in a one-off bounded custom 7-way overlap attempt on the current codebase (requested instance concurrency 7, peak active runs observed 7) and carrying the full matrix report plus per-run raw reports/tx logs.

## Still not proven locally

- Host-local parallel coverage is now real but still bounded: the strongest preserved raw proof bundle reaches 7 overlapping isolated harness + Anvil instances on one machine (`packages/foundry/proof/local/20260316-host-local-saturation-c7-proof/`), while the checked-in compact proof pack still copies through 6; heavier 8-10 deployment saturation beyond that lower bound remains unproven locally.
